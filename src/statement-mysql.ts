    ...
    SELECT CONCAT(
        ...
        CASE
            WHEN COLUMN_DEFAULT IS NOT NULL THEN
                CASE
                    -- EXTRA에 DEFAULT_GENERATED가 포함된 경우 표현식이므로 QUOTE 없이 사용
                    WHEN EXTRA LIKE '%DEFAULT_GENERATED%' THEN
        ...
        CASE
            WHEN EXTRA != '' THEN
                -- EXTRA에서 'DEFAULT_GENERATED'만 제거하고 나머지는 유지
                -- 예: 'DEFAULT_GENERATED on update CURRENT_TIMESTAMP(3)' -> ' on update CURRENT_TIMESTAMP(3)'
                -- TRIM으로 앞뒤 공백 제거
                CASE
                    WHEN TRIM(REPLACE(EXTRA, 'DEFAULT_GENERATED', '')) != '' THEN
  ...
  for (const tableName in comments) {
    ...
    if (table) {
      ...
      commentStatements.push(
        `ALTER TABLE \`${table.tableName}\` COMMENT = ${toStringLiteral(
          table.comment
        )};`
      );
    }
    ...
    if (columns) {
      for (const column of columns) {
        ...
        commentStatements.push(
          `CALL prisma_update_column_comment(${toStringLiteral(
            column.tableName
          )}, ${toStringLiteral(column.columnName)}, ${toStringLiteral(
            column.comment
          )});`
        );
